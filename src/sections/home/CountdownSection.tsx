...
export function CountdownSection() {
  const { settings } = useParticipant();
  const countdown = useCountdown(settings.giveawayEndIso);

  useEffect(() => {
    void trackCampaignEvent("countdown_view");
  }, []);
          ...
          <div className="relative flex flex-col items-center justify-between gap-5 border-b border-white/[0.07] pb-6 sm:flex-row">
            <div className="flex items-center gap-3 text-center sm:text-left">
              ...
              <div>
                ...
                <time dateTime={settings.giveawayEndIso} className="mt-1 block text-xs text-muted">
                  {settings.winnerAnnouncementDate} · {settings.winnerAnnouncementTime}
                </time>
              </div>
            </div>
