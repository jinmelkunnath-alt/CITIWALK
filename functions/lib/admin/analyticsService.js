...
async function getAdminAnalytics() {
    const [participantSnapshot, verificationSnapshot] = await Promise.all([
        firebase_1.db.collection("participants").orderBy("timestamp", "desc").limit(20_000).get(),
        firebase_1.db
            .collection("logs")
    ...
    for (const document of participantSnapshot.docs) {
        ...
        const timestamp = data.timestamp instanceof firestore_1.Timestamp ? data.timestamp.toDate() : null;
        if (timestamp) {
        ...
        increment(browsers, (0, browserUtils_1.detectBrowser)(String(data.userAgent || "")));
    }
    ...
    return {
        ...
        verification: {
            instagramAttempts,
            instagramSuccesses,
            instagramSuccessRate: instagramAttempts > 0
                ? Math.round((instagramSuccesses / instagramAttempts) * 100)
                : 0,
            whatsappAttempts,
            whatsappSuccesses,
            whatsappSuccessRate: whatsappAttempts > 0
